...
object JdbcUtils extends Logging {
    ...
    val columns = if (tableSchema.isEmpty) {
      ...
      rddSchema.fields.map { col =>
        val normalizedName = tableColumnNames.find(f => columnNameEquality(f, col.name)).getOrElse {
          throw new RuntimeException( s"""Column "${col.name}" not found in schema $tableSchema""")
        }
  ...
  private def getCatalystType(
    ...
    val answer = sqlType match {
      ...
      case java.sql.Types.DECIMAL
        if precision != 0 || scale != 0 => DecimalType(scala.math.min(precision, DecimalType.MAX_PRECISION), scala.math.min(scale, DecimalType.MAX_SCALE))
  ...
  def resultSetToRows(resultSet: ResultSet, schema: StructType): Iterator[Row] = {
    ...
    val internalRows = resultSetToSparkInternalRows(resultSet, schema, null)
